...
status=`postmap -q techtalik.co mysql:/etc/postfix/mysql-virtual-mailbox-domains.cf`
if [ $status -ne 1 ]; then
...
status=`postmap -q umair@techtalik.co mysql:/etc/postfix/mysql-virtual-mailbox-maps.cf`

status=`postmap -q umair@techtalik.co mysql:/etc/postfix/mysql-virtual-mailbox-domains.cf`
if [ $status -ne 1 ]; then
